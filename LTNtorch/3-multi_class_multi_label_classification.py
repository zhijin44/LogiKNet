...
train_data = features[:160].float().to(device)
test_data = features[160:].float().to(device)
train_sex_labels = labels_sex[:160].long().to(device)
test_sex_labels = labels_sex[160:].long().to(device)
train_color_labels = labels_color[:160].long().to(device)
test_color_labels = labels_color[160:].long().to(device)
...
def compute_accuracy(loader, threshold=0.5):
    ...
    for data, labels_sex, labels_color in loader:
        # 确保数据在正确的设备上
        data, labels_sex, labels_color = data.to(device), labels_sex.to(device), labels_color.to(device)
